@foreach ($penjualans as $key => $item)
    <div class="modal fade" id="detailPenjualan-{{ $item->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="detailPenjualan-{{ $item->id }}Label" aria-hidden="true">
                            ...
                            <div class="row">
                                <div class="col-xl-12">
                                    <ul class="list-unstyled">
                                        <li>
                                            <h5><i class="ri-checkbox-circle-fill text-primary"></i><strong> Nota : {{ $item->nota }}
                                                    @if ($item->type_pembayaran == 'cash')
                                                        <span class="badge bg-success">Cash</span>
                                                    @elseif ($item->type_pembayaran == 'transfer')
                                                        <span class="badge bg-info">Transfer</span>
                                                    @elseif ($item->type_pembayaran == 'piutang')
                            ...
                            <div class="row my-2 mx-1 justify-content-center">
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            @foreach ($item->detailPenjualans as $key => $detail)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                    ...
                                    <div class="row">
                                        <div class="col-xl-7">
                                            <textarea class="form-control" name="keterangan" readonly>{{ $item->keterangan ?? '-' }}</textarea>
                                        </div>
                                        <div class="col-xl-5">
                                            <ul class="list-unstyled">
                                    ...
                                    @if ($item->type_pembayaran == 'piutang')
                                        <table class="table table-hover mt-5">
                                            ...
                                            <tbody>
                                                @foreach ($item->bayarHutangs as $data)
                                                    <tr>
                                                        <th scope="row">1</th>
                                    ...
                                    <div class="text-center">
                                        @if ($item->type_pembayaran == 'piutang')
                                            <button type="button" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#modalBayarUang-{{ $item->id }}">
                                                Bayar <span><i class="ri-cash-fill"></i></span>
                                            </button>
                                        @endif
                                    </div>
                                </div>
