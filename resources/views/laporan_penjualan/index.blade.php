        ...
        <div class="card-body p-3">
            <div class="row gx-4 d-flex justify-content-between">
                <div class="col-12 col-md-auto d-flex align-items-center mb-3 mb-md-0">
                    ...
                    <h5 class="mb-0 mt-0"><strong>Laporan Penjualan</strong></h5>
                </div>
                <div class="col-12 col-md-auto d-flex align-items-center">
                    <form action="{{ route('laporan.penjualan') }}" method="GET" enctype="multipart/form-data">
                        @csrf
                        <div class="row gx-2">
                                ...
                                <td>
                                    <div style="border: 1px solid #ccc; padding: 10px; width: 100%; border-radius: 10px; display: flex; flex-direction: column;">
                                        @foreach ($data->detailPenjualans as $detail)
                                            {{ $detail->jumlah_keluar ?? '-' }} {{ $detail->produks->nama_produk ?? '-' }} = Rp {{ number_format($detail->total_harga, 0, ',', '.' ?? '-') }}<br>
                                        @endforeach
                                    </div>
                                </td>
                                <td class="text-center"><a href="{{ route('print.penjualan', ['id' => $data->id]) }}" target="_blank" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="right" title="Cetak nota"><i class="ri-printer-fill"></i></a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
